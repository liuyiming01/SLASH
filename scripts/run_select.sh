...
run_graphsst_for_model () {
  ...
  for ds in "${SST_TASKS[@]}"; do
    python -m graphlens.entropy3 \
      --model_path "${MODEL_PATH}" --task_name "${ds}" --data_dir "${GRAPHSST_ROOT}" \
      --output_dir "${ENTROPY_OUT}" --input_column "input_prompt" \
      --sample_num "${SAMPLE_NUM}" --max_seq_len "${MAX_SEQ_LEN}" --hard_max_edges 150 \
      --score_mode "${MODE}" --alpha "${ALPHA}" --select_top_fraction "${SELECT_TOP_FRACTION}" \
      --split "test"
    ...
    python -m graphlens.final_select \
      --mode "${MODE}" \
      --json_entropy "${ENTROPY_OUT}/${ds}_entropy_${MODE}_alpha${ALPHA}/${ds}_selected_layers_middle_peak_entropy.json" \
      --json_scoring "${SCORE_DIR}/${ds}_selected_layers_auto_scoring.json" \
      --output_json "${SELECT_OUT}/${ds}_${MODE}_intersection.json"
  done
}

# 1) MODELS: run all tasks
for MODEL_PATH in "${MODELS[@]}"; do
  run_graphwiz_for_model "${MODEL_PATH}"
  # run_graphsst_for_model "${MODEL_PATH}"
done
